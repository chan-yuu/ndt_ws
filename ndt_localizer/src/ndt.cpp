 ...
 * @FilePath: /undefined/home/cyun/learn_ws/ndt_ws/src/ndt_localizer/src/ndt.cpp
 * @Description: 
 * 
...
void NdtLocalizer::processPointCloudData() {
    if (!sensorPointsSensorTF.data.empty()) {
      const auto exe_start_time = std::chrono::system_clock::now();
        ...
        // key_value_stdmap_["seq"] = std::to_string(sensorPointsSensorTF.header.seq);
        // key_value_stdmap_["transform_probability"] = std::to_string(transform_probability);
        // key_value_stdmap_["iteration_num"] = std::to_string(iteration_num);
        // key_value_stdmap_["skipping_publish_num"] = std::to_string(skipping_publish_num);
...
void NdtLocalizer::init_params() {
  ...
  private_nh_.getParam("location_mode", location_mode);
